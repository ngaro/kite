...
ac=0.32
theta = np.pi/3.0
a1 = ac*np.array([(1 + np.cos(theta)),np.sin(theta)])
a2 = ac*np.array([0, 2*np.sin(theta)])

xmin, xmax, Nx = 40*ac, 55*ac, 300
ymin, ymax, Ny = 75*ac, 90*ac, 300
gridxs = np.linspace(0,xmax-xmin,Nx)
gridys = np.linspace(0,ymax-ymin,Ny)
...
def process_orb(ORB):
    ...
    for i in range(count):
        ...
        if xmin <= x <= xmax and ymin <= y <= ymax:
            print(x,y)
            exponent = -((meshx - (x-xmin))**2 + (meshy - (y-ymin))**2)/(variance*ac*ac)
...
plt.xlabel(r'x(nm)', labelpad=5, size=50)
plt.ylabel(r'y(nm)',size=50)
xticks = np.linspace(0,xmax-xmin,10)
